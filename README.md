...
- **[AIVideoRunbook](https://github.com/madebysaira/AIVideoRunbook)** – Offline-first CLI that turns the AIVideo* safety-net gates (CreditGuard + QualityGate + AdherenceGate) into one credit-to-delivery pipeline. Runs the gates in order, estimates real cost (sticker × retries), and logs every generation to a local SQLite catalog keyed by prompt, model, seed, and verdict. Exit codes `0/1/2` for n8n/xyOps; queryable catalog replaces spreadsheets. Built for creators who burn ~$600 in failed Kling/Veo runs before they learn a clip was wrong. [Docs](https://github.com/madebysaira/AIVideoRunbook)
